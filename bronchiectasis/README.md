...
兩個獨立的單檔網頁工具，都是零外連、資料不離開本機：

| 檔案 | 用途 | 大小 | 資料保存 |
|---|---|---|---|
| [`bronchiectasis-registry.html`](../bronchiectasis-registry.html) | **個案登錄簿** — 手動 key 病歷號與臨床資料，供科內記錄與討論會 | 60 KB | 密碼加密（AES-256-GCM），可存加密檔 |
| [`bronchiectasis-dashboard.html`](../bronchiectasis-dashboard.html) | **PDF 儀錶板** — 上傳門診紀錄 PDF 自動擷取，產生一頁式報告 | 2.1 MB | 完全不保存，關分頁即消失 |

兩者的安全模型刻意不同，所以是兩個檔案：儀錶板不碰任何儲存空間；
登錄簿因為要累積個案，改用密碼加密保存。

---

# 一、個案登錄簿 (bronchiectasis-registry.html)

手動登錄個案，即時計算 BSI / FACED / E-FACED，並記錄討論重點、決議與待辦。
...
| 方式 | 說明 |
|---|---|
| **加密保存**（建議） | 設一組密碼（≥8 字元），以 PBKDF2-SHA256（60 萬次迭代）派生 AES-256-GCM 金鑰。偵測到瀏覽器儲存可用時自動保存，隨時可另存加密檔 |
| **開啟加密檔** | 載入先前存下的備份，換機器或給同事討論時用 |
| **只用這次** | 不設密碼、不寫入任何儲存空間，載入範例資料供試用 |

閒置 15 分鐘自動鎖定，金鑰只存在記憶體（`extractable: false`），鎖定即清除。

## 登錄欄位

個案識別（自動編號 BE-001…、病歷號、姓名縮寫、性別、年齡、診斷日期、病因）、
體位與肺功能（身高體重自動換算 BMI、FEV₁ / FVC %pred、FEV₁/FVC）、
症狀與病程（mMRC 含中文描述、痰量、咳血、近 1 年發作與住院、近 2 年住院）、
影像（六個肺葉、柱狀 / 靜脈曲張 / 囊狀）、
微生物（10 種菌 + 慢性定殖判定）、治療（11 項）、
追蹤紀錄（可新增多筆日期 / FEV₁ / 發作）、
討論紀錄（討論重點、決議、待辦、下次追蹤）。

## 匯出

- **加密備份檔**（`.json`）— 含病歷號，需原密碼開啟
- **去識別 CSV** — 只保留個案編號與臨床欄位，無病歷號與姓名，
  並對以 `=` `+` `-` `@` 開頭的欄位加單引號跳脫（防 Excel 公式注入），
  加上 UTF-8 BOM 讓 Excel 正確辨識中文

兩者都提供「複製全部」，因為部分環境（如 artifact 檢視器）不允許網頁觸發下載。

## 安全設計

與儀錶板相同的零外連與 CSP 設定，另加：

- 病歷號在清單中預設遮蔽（`範••A`），可切換顯示
- 資料寫入儲存空間前一律加密；實測密文中不含病歷號明碼
- 每次加密使用新的隨機 IV（AES-GCM 重用 IV 會失效）
- 全程 `textContent` / `createElement`，無 `innerHTML`，無 inline style 屬性
- 確認對話框自建於頁面內（`confirm()` 在部分檢視器不會顯示）

---

# 二、PDF 儀錶板 (bronchiectasis-dashboard.html)
